...
func makeEntry(date string, value int) models.RoutineEntry {
	return models.RoutineEntry{Date: date, Value: value}
}

func entries(dates ...string) []models.RoutineEntry {
	var out []models.RoutineEntry
	for _, d := range dates {
...
func TestCurrentStreakEmpty(t *testing.T) {
	r := models.Routine{Frequency: "daily", TargetValue: 1}
	got := steward.CurrentStreak(nil, r, "2026-05-18")
	if got != 0 {
...
func TestCurrentStreakTodayDone(t *testing.T) {
	...
	got := steward.CurrentStreak(entries, r, "2026-05-18")
	if got != 4 {
...
func TestCurrentStreakSingleEntry(t *testing.T) {
	r := models.Routine{Frequency: "daily", TargetValue: 1}
	got := steward.CurrentStreak(entries("2026-05-18"), r, "2026-05-18")
	if got != 1 {
...
func TestLongestStreakConsecutive(t *testing.T) {
	...
	got := steward.LongestStreak(entries, r)
	if got != 5 {
...
func TestLongestStreakEmpty(t *testing.T) {
	r := models.Routine{Frequency: "daily", TargetValue: 1}
	got := steward.LongestStreak(nil, r)
	if got != 0 {
		t.Errorf("expected 0 for empty entries, got %d", got)
	}
}
...
func TestCurrentStreakWeekly(t *testing.T) {
	r := models.Routine{Frequency: "weekly", TargetValue: 1}
	...
	got := steward.CurrentStreak(entries, r, "2026-05-13")
	if got != 1 {
...
func TestCurrentStreakWeeklyMultiWeek(t *testing.T) {
	...
	got := steward.CurrentStreak(entries, r, "2026-05-12")
	if got != 3 {
...
func TestCurrentStreakMonthly(t *testing.T) {
	r := models.Routine{Frequency: "monthly", TargetValue: 1}
	entries := []models.RoutineEntry{
...
func TestWeeklyStreakAcrossISOYearBoundary(t *testing.T) {
	...
	got := steward.CurrentStreak(entries, r, "2025-12-30")
	if got != 2 {
